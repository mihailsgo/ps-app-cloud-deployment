...
sequenceDiagram
  ...
  User->>SPA: Login + open document
  SPA->>KC: OIDC authentication
  SPA->>API: GET /latestUser (Bearer)
  ...
  SPA->>API: PUT /visual-signature {docid,...}
  ...
  SPA->>API: POST /stamp {docid}
  API->>ARC: Download latest signed PDF
  ARC-->>API: PDF bytes
  API->>ESEAL: POST multipart PDF + stamp headers
