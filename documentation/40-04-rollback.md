...
### Rehearsed on real Linux

Run on 2026-09-25 on WSL Ubuntu's own Docker engine (28.3.2, Compose v2.38.2, classic overlay2 image store) as a normal user in the `docker` group, in a fresh clone under its own compose project, with an untracked `docker-compose.override.yml` that renamed every container and removed every host port, and a self-signed certificate (`--allow-self-signed`). Cosign was not installed, so the signature pre-flight only warned. Torn down with `docker compose down -v` afterwards.

| Step | Result |
|---|---|
| `bootstrap.sh` at `7d8f643` | 3.28 / 8.39 running, all services healthy |
| `git stash; git pull; git stash pop` to `main` (`653f046`), clean | `docker-compose.yml` pins 3.30 / 8.40, containers still 3.28 / 8.39 |
| `main`'s `upgrade.sh --server-tag 3.30 --client-tag 8.40` (`--plan-only`, then real) | `ps-server: 3.30 → 3.30`; manifest schema 1 with `image_tags` 3.30 / 8.40 and digests `98bb0577…` / `8722d407…`; `.rollback-snapshots/` `755`, `config.js` copy `644` |
| `main`'s `rollback.sh --yes` | `ps-server -> 3.30@sha256:cdcabd7d…`, `Rollback complete`, exit 0, stack still 3.30 / 8.40: **the bug, reproduced** |
| stash/pull/pop to this change, then its `rollback.sh --yes` against that same schema-1 snapshot | two `WARNING`s (compose copy pinned 3.30 / 8.40, the running containers were 3.28 / 8.39, tag from `release/unsigned-legacy-images.json`), restored `3.28@98bb0577…` / `8.39@8722d407…`, verified, exit 0; `validate-config.sh` exit 0 with the WARN lines shown in the next section |
| `docker-compose.yml`'s two image lines set back to the pulled release's (the post-pull state), then this change's `upgrade.sh`, plan and real | `ps-server: 3.28 → 3.30` plus the `NOTE` line; machine plan `server_tag_from=3.28`, `server_tag_pinned=3.30`; manifest schema 2 recording 3.28 / 8.39 and their digests, `compose_pins` 3.30 / 8.40; the old snapshot tightened to `700`/`600` |
| this change's `rollback.sh --yes` | `3.30 -> 3.28@sha256:98bb0577…`, `8.40 -> 8.39@sha256:8722d407…`, both verified; `docker inspect` of the new containers: `mihailsgordijenko/ps-server:3.28@sha256:98bb0577…`, `mihailsgordijenko/ps-client:8.39@sha256:8722d407…`; `validate-config.sh` exit 0 |
| `rollback.sh --yes` again as root, then again as the operator | both exit 0, no change; root reads the operator's mode-`700` snapshots |
| `upgrade.sh --server-tag 3.30 --client-tag 8.40` (roll forward) | `.rollback-applied.json` removed; `validate-config.sh` OK for both pins again |
...
For the rollback from 3.30 / 8.40 to `3.28@98bb0577…` / `8.39@8722d407…` that means (the ps-server lines from the rehearsal above; ps-client reads the same; `b888336` is the last commit whose `release/approved-digests.json` approved 3.28 / 8.39):

```
  WARN Release snapshot ps-server (3.30) != docker-compose (3.28): rollback.sh restored 3.28 - see Image digest pinning below
  WARN ps-server: rolled back to mihailsgordijenko/ps-server:3.28 by rollback.sh (snapshot 20260925T123124Z, 2026-09-25T12:32:19Z) - approved by release commit b888336aebaa, not by this checkout's release/approved-digests.json (which approves 3.30). Roll forward with upgrade.sh --server-tag 3.30 once the cause of the rollback is fixed
  WARN ps-server (3.28): released before image signing existed; exempt by exact digest in release/unsigned-legacy-images.json
  OK   ps-server running correct tag (3.28)
All checks passed.
```
...
`upgrade.sh`, `rollback.sh` and `bootstrap.sh` now call `wait_for_healthy` (`installation-scripts/lib/health-wait.sh`) on the services they (re)started (plus `nginx`, which they restart). It returns as soon as every one is healthy, and fails fast when one reports `unhealthy`, has exited, or restarts twice while waiting (crash loop); otherwise it times out (`--health-timeout`, default 480s since v1.0.47, before that 300s; 600s for `bootstrap.sh`, whose first Keycloak boot is slow). The default is never shorter than the longest health-check window in `docker-compose.yml` (450 s for the DMSS JVMs, see [40.1](40-01-health-checks-and-startup-order.md#start-up-windows)). On failure it prints the service, its last health-probe output and its last 15 log lines.
