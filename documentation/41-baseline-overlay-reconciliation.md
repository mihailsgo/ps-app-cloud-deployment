...
| `config/config.js` | hostname-embedded service URLs, Keycloak backend `secret`, the `REGISTER_PDF_API_KEY` / `SESSION_SECRET` bootstrap generates, `DEMO_COMPANY_ROLE`, the `DOCUMENT_ROUTING` block, `STAMP_MODE`/`STAMP_LOCAL` |
| `docker-compose.yml` | `ps-server`/`ps-client` image tags, the `signed-output` volume mount, `KEYCLOAK_ADMIN` (and an inline `KEYCLOAK_ADMIN_PASSWORD`, on deployments bootstrapped before v1.0.42 only: later ones keep it in `.env`), keycloak's `KC_HOSTNAME`, nginx's network alias, the optional local-eseal service block + its `SPRING_SECURITY_USER_*` env |
