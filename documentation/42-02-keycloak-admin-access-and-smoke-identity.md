...
| `KEYCLOAK_ADMIN_PASSWORD` in compose | Nothing to do: it is read only on Keycloak's **first** boot against an **empty** volume. After the 42.4 migration the new checkout's compose reads it from `.env` (`KEYCLOAK_FIRST_BOOT_ADMIN_PASSWORD`), and `overlay.sh capture` deliberately does not carry that key (listed under *NOT captured*). `validate-config.sh` warns that it falls back to the demo default `admin`. | the warning is expected; the real credential lives in the Keycloak volume and your secret manager |
