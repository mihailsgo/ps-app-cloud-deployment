...
| Monitoring alerts on repeated restarts, unhealthy services, certificate risk, stamping/archive failures, disk pressure, and unacknowledged buffer growth | Done - `monitor-status.sh --alert` from cron, delivering to `ALERT_WEBHOOK_URL` (Slack, any HTTP receiver, or Microsoft Teams Workflows with `ALERT_WEBHOOK_FORMAT=teams`). See [40.3](40-03-monitoring-and-alerting.md). Each deployment still has to set the URL and install the cron entry; `monitor-status.sh --test-webhook` checks the URL. |
