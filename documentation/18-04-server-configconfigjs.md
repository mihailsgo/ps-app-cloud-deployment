...
- `REGISTER_PDF_API_KEY`: API key used by external integrations for API-key protected endpoints. `bootstrap.sh` replaces the value shipped in this repository with a random one and does not print it; [18.5](18-05-cloud-flow-apiregisterpdf.md#reading-the-api-key) shows how to read it.
- `ALLOW_INSECURE_TLS`: Optional TLS relaxation for troubleshooting only (keep `false` in production).
- `SESSION_SECRET`: Session secret for backend internals. Also generated by `bootstrap.sh` when it still holds the shipped value. Changing it only ends existing sessions.
